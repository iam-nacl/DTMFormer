...
class JointTransform2D_BTCD2D:
    ...
    def __init__(self, img_size=256, resize=256, crop=(32, 32), p_flip=0.0, p_rota=0.0, p_scale=0.0, p_gaussn=0.0, p_contr=0.0,
                 p_gama=0.0, p_distor=0.0, z_score=False, color_jitter_params=(0.1, 0.1, 0.1, 0.1), p_random_affine=0,
                 long_mask=False):
        self.crop = crop
        self.resize = resize
    ...
    def __call__(self, image, mask):
        ...
        # resize
        if self.resize:
            _resize = torchvision.transforms.Resize(self.resize, interpolation=F.InterpolationMode.BICUBIC)
            image, mask = _resize(image), _resize(mask)
...
class ImageToImage2D_Substation(Dataset):
    ...
    def __getitem__(self, i):
        id_ = self.ids[i]
        # image = cv2.imread(os.path.join(self.img_path, id_ + '.png'), 0)
        filename1 = os.path.join(self.img_path, id_ + '.jpg')  # 或者 .jpeg
        filename2 = os.path.join(self.img_path, id_ + '.jepg')  # 或者 .jpeg
...
class ImageToImage2D_BTCV2D(Dataset):
    """
    Reads the images and applies the augmentation transform on them.
    Usage:
        1. If used without the unet.model.Model wrapper, an instance of this object should be passed to
